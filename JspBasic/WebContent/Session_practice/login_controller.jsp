...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    <%
	    ...
	    null값을 받았다면 회원가입이 되어있지 않은 사람이 로그인을 시도한 것이므로
	        로그인 페이지로 보내주세요. (script로 경고창 띄운 후 로그인 페이지로.)
	    ...
	    */
	    ...
	    //로그인을 시도하는 회원의 객체를 받아오기. (변수로 미리 선언해놓는 것이 좋음)
	    User user = UserRepository.getUser(id);
	    
    	if(user != null) {
    		if(pw.equals(user.getPassword())) { //로그인 성공.
    			session.setAttribute("login", user); //세션 만들기.
    			response.sendRedirect("login_welcome.jsp"); //웰컴페이지로 보내주기.
    		} else { //로그인 실패 (비밀번호) %>
    			<script>
